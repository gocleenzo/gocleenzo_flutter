...
class _ServicePricing {
  static const Map<String, Map<String, dynamic>> config = {
    ...
    'Full House Cleaning': {
      ...
      'durations': {'1 BHK': 90,  '2 BHK': 120,  '3 BHK': 150},
    },
    'Kitchen Cleaning':            {'type': 'fixed', 'duration': 60},
    'Kitchen Cabinet Cleaning':    {'type': 'fixed', 'duration': 150},
    'Utensil Cleaning':            {'type': 'fixed', 'duration': 45},
    'Wardrobe Cleaning':           {'type': 'fixed', 'duration': 120},
...
class _ServiceDetailScreenState extends State<ServiceDetailScreen>
  ...
  // Pricing state
  int    _quantity     = 1;
  int    _selectedHours = 1; // for hourly type (min 1, max 6)
  ...
  // ── Exact service.id → local asset mapping ────────────────────
  ...
  Map<String, dynamic> get _pricing {
    ...
    // Hourly — detected by pricing_type column = 'hourly'
    ...
    if (hasBhkPrices) {
      return {
        ...
        'durations': {
          '1 BHK': (svc['duration_1bhk'] as num?)?.toInt() ?? 60,
          '2 BHK': (svc['duration_2bhk'] as num?)?.toInt() ?? 90,
          '3 BHK': (svc['duration_3bhk'] as num?)?.toInt() ?? 120,
        },
      };
    }
  ...
  int get _originalPrice {
    ...
    switch (p['type'] as String) {
      ...
      case 'hourly':  return (p['price_per_hour'] as int) * _selectedHours;
      default:         return (_service?['base_price'] as num?)?.toInt() ?? 0;
    }
  }

  /// Services eligible for ₹25 first booking offer
  ...
  int get _computedPrice {
    ...
    final type = _pricing['type'] as String;
    return type == 'per_unit' ? 25 * _quantity : 25;
  }
  ...
  int get _computedDuration {
    final p = _pricing;
    // Hourly: duration IS the selected hours — no buffer needed
    if (p['type'] == 'hourly') return _selectedHours * 60;
    int exactMins;
    switch (p['type'] as String) {
      case 'per_unit':
        exactMins = (p['duration_per_unit'] as int) * _quantity;
      case 'by_bhk':
  ...
  // Display duration — no buffer, no rounding (customer-facing only)
  int get _displayDuration {
    final p = _pricing;
    if (p['type'] == 'hourly') return _selectedHours * 60;
    switch (p['type'] as String) {
      case 'per_unit':
        return (p['duration_per_unit'] as int) * _quantity;
      case 'by_bhk':
  ...
  Future<void> _navigate(String mode) async {
    ...
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => BookingFlowScreen(
        ...
        quantity:         _pricing['type'] == 'per_unit' ? _quantity
                          : _pricing['type'] == 'hourly' ? _selectedHours
                          : null,
      ...
      for (final w in workers) {
        ...
        for (final bk in active) {
          ...
          final bDt = DateTime.tryParse(
              bk['scheduled_at'].toString())?.toLocal();
          ...
          if (slotDt.isBefore(bEnd) && slotEnd.isAfter(bDt)) {
            busy = true; break;
          }
        }
        if (!busy) return null;
      ...
      builder: (ctx) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const Text(
              'Instant bookings are only available\nbetween 7:00 AM - 7:00 PM.',
              textAlign: TextAlign.center,
              style: TextStyle(
                  color: Color(0xFF64748B), fontSize: 13.5, height: 1.5)),
              ...
              onTap: () {
                ...
                Navigator.push(context, MaterialPageRoute(
                  builder: (_) => BookingFlowScreen(
                    ...
                    overridePrice:    _computedPrice,
                    overrideDuration: _computedDuration,
                    selectedBhk: _pricing['type'] == 'by_bhk'
                        ? _selectedBhk : null,
                    quantity: _pricing['type'] == 'per_unit' ? _quantity
                        : _pricing['type'] == 'hourly' ? _selectedHours
                        : null,
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(14),
                  border: Border.all(color: const Color(0xFFE8EDF2))),
                child: const Text('Try After Some Time',
                    style: TextStyle(color: Color(0xFF64748B),
                        fontSize: 14, fontWeight: FontWeight.w700)),
              ),
            ),
  ...
  Widget _buildUnitSelector(Map<String, dynamic> p) {
    ...
    final durationPerUnit = p['duration_per_unit'] as int;
    final rawTotalMins    = durationPerUnit * _quantity;

    if (_quantity > max) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) setState(() => _quantity = max);
      });
    } else if (_quantity < min) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) setState(() => _quantity = min);
      });
    }
      ...
      child: Column(children: [
        ...
        Container(
          ...
          child: Row(
              ...
              children: [
            Row(children: [
              const Icon(Icons.timer_outlined, size: 14, color: _cyanDk),
              const SizedBox(width: 6),
              Text('$_quantity × ${durationPerUnit}min',
                  style: const TextStyle(color: _muted, fontSize: 13)),
            ]),
            Text('= ${rawTotalMins}min',
                style: const TextStyle(fontWeight: FontWeight.w900,
                    fontSize: 15, color: _ink)),
          ]),
        ),
  ...
  Widget _buildHourlySelector(Map<String, dynamic> p) {
    final pph  = p['price_per_hour'] as int;
    final maxH = p['max_hours'] as int;
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 16),
        Row(children: List.generate(maxH, (i) {
          final h      = 1 + i;
          final active = _selectedHours == h;
          return Expanded(child: Padding(
            padding: EdgeInsets.only(right: i < maxH - 1 ? 6 : 0),
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Column(children: [
                  Text('${h}hr', style: TextStyle(
                      fontWeight: FontWeight.w900, fontSize: 13,
                      color: active ? Colors.white : _ink)),
                  const SizedBox(height: 2),
                  Text('₹${pph * h}', style: TextStyle(
        ...
        Container(
          ...
          child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
            Text('$_selectedHours hr × ₹$pph',
                style: const TextStyle(color: _muted, fontSize: 13)),
            Text('= ₹${pph * _selectedHours}',
                style: const TextStyle(fontWeight: FontWeight.w900,
                    fontSize: 15, color: _ink)),
          ]),
        ),
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
        Expanded(child: Column(
          ...
          Container(
            ...
            child: Text(
              isFirst
                  ? '🎉 ₹${original - display} off for first booking'
  ...
  List<Map<String, dynamic>> _expectItems() {
    return [
      ...
      {'text': '✓ ~$_displayDuration min estimated duration',
        'color': const Color(0xFF000000)},
    ];
  }
  ...
  Widget _buildAbout(Map<String, dynamic> svc) {
    final desc = svc['description'] as String? ??
        'Professional cleaning by verified experts.';

    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(desc, style: const TextStyle(color: _ink,
      ...
      GridView.count(
        ...
        children: [
          _whyCard('🛡️', 'Instant Booking',
              const Color(0xFFECFEFF), const Color(0xFF0891B2)),
          _whyCard('⏱️', 'House Help in Minutes',
              const Color(0xFFECFEFF), const Color(0xFF0891B2)),
          _whyCard('🔄', 'Safe And Trusted',
              const Color(0xFFECFEFF), const Color(0xFF0891B2)),
          _whyCard('💳', 'Verified and trained Staff',
              const Color(0xFFECFEFF), const Color(0xFF0891B2)),
        ],
      ),
    ]);
            ...
            if (_pricing['type'] != 'fixed')
              Text(
                _pricing['type'] == 'per_unit'
                    ...
                    : _pricing['type'] == 'hourly'
                        ? '$_selectedHours hr · ₹${_pricing['price_per_hour']}/hr'
                        : _selectedBhk,
